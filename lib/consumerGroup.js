    ...
    async.waterfall(
      [
        ...
        function (offsets, callback) {
          ...
          self.notCommitedTopicPayloads = _.cloneDeep(self.topicPayloads);
...
ConsumerGroup.prototype.fetch = function () {
  if (!this.ready || this.rebalancing || this.closing) {
    return;
  }
...
function autoCommit (options, cb) {
  ...
  if (commits.length) {
    ...
    this.notCommitedTopicPayloads = _.cloneDeep(topicPayloads);
  } else {
    cb(null, 'Nothing to be committed');
  }
}
